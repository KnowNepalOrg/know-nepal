package com.knownepal.history.specification;
...
class SpecificationFactoryTest {
    ...
    void buildSpec_withEmptyFilters_returnsConjunction() {
        ...
        var spec = SpecificationFactory.<HistoricalEvent>buildSpec(filters);

        Root<HistoricalEvent> root = mock(Root.class);
    ...
    void buildSpec_withNullFilters_returnsConjunction() {
        var spec = SpecificationFactory.<HistoricalEvent>buildSpec(null);
    ...
    void buildSpec_withEnumFilter_addsEqualPredicate() {
        Map<String, String> filters = Map.of("eventType", "BATTLE");
        var spec = SpecificationFactory.<HistoricalEvent>buildSpec(filters);
        ...
        when(path.getJavaType()).thenReturn(EventType.class);
        when(cb.equal(path, EventType.BATTLE)).thenReturn(equalPredicate);
        when(cb.conjunction()).thenReturn(conjunction);
        when(cb.and(any(Predicate[].class))).thenReturn(equalPredicate);

        Predicate result = spec.toPredicate(root, query, cb);

        verify(cb).equal(path, EventType.BATTLE);
    }

    @SuppressWarnings("unchecked")
    @Test
    void buildSpec_withBlankValue_skipsFilter() {
        Map<String, String> filters = Map.of("eventType", "  ");
        var spec = SpecificationFactory.<HistoricalEvent>buildSpec(filters);
    ...
    void buildSpec_withStringFilter_usesLike() {
        Map<String, String> filters = Map.of("name", "battle");
        var spec = SpecificationFactory.<HistoricalEvent>buildSpec(filters);
        ...
        when(cb.like(lowered, "%battle%")).thenReturn(likePredicate);
        when(cb.conjunction()).thenReturn(conjunction);
        when(cb.and(any(Predicate[].class))).thenReturn(likePredicate);

        Predicate result = spec.toPredicate(root, query, cb);

        verify(cb).like(lowered, "%battle%");
    }
}
